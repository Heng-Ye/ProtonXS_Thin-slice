#define ProtonESliceData_cxx
#include "ProtonESliceData.h"
...
void ProtonESliceData::Loop() {
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) { //main entry loop
		...
		true_sliceID = int(Len2KE(range_true, ke_ff)/thinslicewidth); 
		...
		if (IsPandoraSlice&&IsCaloSize) {
			...
			reco_sliceID = int(Len2KE(range_reco, ke_ff)/thinslicewidth);
		...
		if (IsPureInEL) { //is pure inelastic
			...
			if (IsCaloSize==true&&IsBeamMatch==true) { //calo & beam_match
				...
				for (size_t ih=0; ih<primtrk_dedx->size(); ++ih) {
					...
					int this_sliceID = int(Len2KE(thisLen, ke_ff)/thinslicewidth);
					//ke_reco-=this_dE;
					ke_reco-=EDept.at(ih);
