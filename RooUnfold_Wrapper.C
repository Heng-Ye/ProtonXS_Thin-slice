...
void RooUnfold_Wrapper() {
	...
	//Output files2 ----------------------------------------------//
        //TString fout2_inc=outpath+"uf_roounfold_svd_inc.root";
        //TString fout2_inc_st=outpath+"uf_roounfold_svd_inc_st.root";
        //TString fout2_int=outpath+"uf_roounfold_svd_int.root";
	...
	TFile *f_out_inc = new TFile(fout_inc.Data(),"RECREATE");
		uf_inc.Write("uf_inc");
		data_inc_uf->Write("data_inc_uf");
	f_out_inc->Close();

	//inc_st
	TFile *f_out_inc_st = new TFile(fout_inc_st.Data(),"RECREATE");
		uf_st_inc.Write("uf_st_inc");
		data_st_inc_uf->Write("data_st_inc_uf");
	f_out_inc_st->Close();

	//int
	TFile *f_out_int = new TFile(fout_int.Data(),"RECREATE");
		uf_int.Write("uf_int");
		data_int_uf->Write("data_int_uf");
	f_out_int->Close();
	//---------------------------------------------------------------------//
