...
void plot_truth_KE() {
	...
	TString file=Form("../mc_truelen_keff.root");
	//TString file=Form("../mc_truelen_old.root");
	//TString file=Form("../mc_truelen.root");
	//TString file=Form("../mc_truelen_truelen_around50.root");
	TFile *fin = TFile::Open(file.Data());

	TH2D *h2d=(TH2D *)fin->Get(Form("h2d_trueZ_ketrue_NoCut_inel"));
	h2d->SetTitle("No cut; True Z [cm]; True Kinetic Energy [MeV]");
	//TH2D *h2d=(TH2D *)fin->Get(Form("h2d_truetrklen_keff_NoCut_inel"));
	//h2d->SetTitle("No cut; True Track Length [cm]; True Kinetic Energy at TPC Front Face [MeV]");
...
*/
	...
	c_->Divide(1,1);
	//c_->SetLogy();
	h2d->GetXaxis()->SetRangeUser(-60, 5);
	h2d->Draw("colz");
	//h2d->ProjectionX()->Draw();
	
	//TString fig_out=Form("plots_bkgstudy/Ztrue_KEtrue_inel.eps");
	//TString fig_out=Form("plots_bkgstudy/Ztrue_inel.eps");
	TString fig_out=Form("plots_bkgstudy/truelen_keff_inel_zoom2.eps");
	c_->Print(fig_out.Data());
