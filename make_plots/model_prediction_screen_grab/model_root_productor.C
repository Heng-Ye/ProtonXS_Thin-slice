void model_root_productor() {
  ...
  f_in_GENIE_ha2018.close();  for (int k=0; k<data_GENIE_ha2018.size(); k+=n) {
  ...
  f_in_NuWRO_2019.close();  for (int k=0; k<data_NuWRO_2019.size(); k+=n) {
    x_NuWRO_2019.push_back(1000.*data_NuWRO_2019[k]); //KE in MeV
  ...
  f_in_GENIE_hN2018.close();  for (int k=0; k<data_GENIE_hN2018.size(); k+=n) {
  ...
  f_in_GENIE_INCL_pp.close();  for (int k=0; k<data_GENIE_INCL_pp.size(); k+=n) {
  ...
  TFile *fout=new TFile("model_pred_screengrab.root","create");
