...
void plot_zreco(TString fin, TString fout_path, TString str_cut) {
	...
	TH1D *z_upinel=(TH1D *)f0->Get(Form("zend_reco_upinel_%s",str_cut.Data()));
	TH1D *z_upel=(TH1D *)f0->Get(Form("zend_reco_upel_%s",str_cut.Data()));
	TH1D *z_upmcs=(TH1D *)f0->Get(Form("zend_reco_upmcs_%s",str_cut.Data()));

	TH1D *z_misinel=(TH1D *)f0->Get(Form("zend_reco_misinel_%s",str_cut.Data()));
	TH1D *z_misel=(TH1D *)f0->Get(Form("zend_reco_misel_%s",str_cut.Data()));
	TH1D *z_mismcs=(TH1D *)f0->Get(Form("zend_reco_mismcs_%s",str_cut.Data()));
