...
void plot_vtx_el_eff() {

	TString file_mc=Form("../mc_vtx_rangereco.root");
	TFile *fmc = TFile::Open(file_mc.Data());

	TH2D *h2d=(TH2D *)fmc->Get(Form("h2d_trklen_dr_RecoEl"));
	TH2D *h2d_el=(TH2D *)fmc->Get(Form("h2d_trklen_dr_RecoEl_el"));
	TH2D *h2d_inel=(TH2D *)fmc->Get(Form("h2d_trklen_dr_RecoEl_inel"));
	TH2D *h2d_misidp=(TH2D *)fmc->Get(Form("h2d_trklen_dr_RecoEl_misidp"));

	TH2D *h2d_true_el=(TH2D *)fmc->Get(Form("h2d_truetrklen_dr_RecoEl_el"));


	//figout
	TString fig_out="plots_vtx/recolen_dr_recoel_zoom.eps";
	TString fig_out_inel="plots_vtx/recolen_dr_recoel_inel_zoom.eps";
	TString fig_out_el="plots_vtx/recolen_dr_recoel_el_zoom.eps";
	TString fig_out_misidp="plots_vtx/recolen_dr_recoel_misidp_zoom.eps";
	...
	c_inel->Divide(1,1);
	c_inel->cd(1);
	h2d->SetTitle("Reco El. Cut; Reco Track Length [cm]; #Delta L");
	h2d->GetYaxis()->SetRangeUser(-10,10);
	h2d->Draw("colz");
	c_inel->Print(fig_out.Data());


	h2d_inel->SetTitle("Reco El. Cut [True Inel.]; Reco Track Length [cm]; #Delta L");
	h2d_inel->GetYaxis()->SetRangeUser(-10,10);
	h2d_inel->Draw("colz");
	c_inel->Print(fig_out_inel.Data());

	h2d_el->SetTitle("Reco El. Cut [True El.]; Reco Track Length [cm]; #Delta L");
	h2d_el->GetYaxis()->SetRangeUser(-10,10);
	h2d_el->Draw("colz");
	c_inel->Print(fig_out_el.Data());

	h2d_misidp->SetTitle("Reco El. Cut [MisID:P]; Reco Track Length [cm]; #Delta L");
