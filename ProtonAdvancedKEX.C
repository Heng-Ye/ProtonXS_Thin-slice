#define ProtonAdvancedKE_cxx
#include "ProtonAdvancedKE.h"
...
/*
	...
	//TH1D *h1d_kebb_reco_Edept_calo_el=new TH1D("h1d_kebb_reco_Edept_calo_el","", nke, kemin, kemax); //

	TH1D *h1d_kebb_reco_Edept_range_inel=new TH1D("h1d_kebb_reco_Edept_range_inel","", nke, kemin, kemax); //
	TH1D *h1d_kebb_reco_Edept_calo_inel=new TH1D("h1d_kebb_reco_Edept_calo_inel","", nke, kemin, kemax); //

	//kecalo
	TH1D *h1d_kecalo_reco_constErange_el=new TH1D("h1d_kecalo_reco_constErange_el","", nke, kemin, kemax); //keff_reco
	TH1D *h1d_kecalo_reco_constErange_inel=new TH1D("h1d_kecalo_reco_constErange_inel","", nke, kemin, kemax); //keff_reco

	TH1D *h1d_kecalo_reco_Edept_range_el=new TH1D("h1d_kecalo_reco_Edept_range_el","", nke, kemin, kemax); //
	TH1D *h1d_kecalo_reco_Edept_calo_el=new TH1D("h1d_kecalo_reco_Edept_calo_el","", nke, kemin, kemax); //

	TH1D *h1d_kecalo_reco_Edept_range_inel=new TH1D("h1d_kecalo_reco_Edept_range_inel","", nke, kemin, kemax); //
	TH1D *h1d_kecalo_reco_Edept_calo_inel=new TH1D("h1d_kecalo_reco_Edept_calo_inel","", nke, kemin, kemax); //
*/
	//------------------------------------------------------------------------------------------------------------------------//
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) { //main entry loop
		...
		if (IsPandoraSlice&&IsBQ&&IsCaloSize) { //if calo size not empty
			...
			for (size_t h=0; h<primtrk_hitz->size(); ++h) { //loop over reco hits of a given track
				...
				double tmp_r0=pow((17./cali_dedx),1./0.42);
				R0.push_back(tmp_r0);
...
/*
		...
		double KEcsda_inel=1000.*ke_vs_csda_range_sm->Eval(av_rr_reco+range_reco)
...
*/
		...
		if (IsPandoraSlice&&IsBQ&&IsCaloSize) { //basic cuts
			...
			Fill1DHist(h1d_keff_reco_constEcalo, ke_beam_spec_MeV-mean_Elosscalo_stop);
			...
			if (kel) { 
				...
				Fill1DHist(h1d_keff_reco_constEcalo_el, ke_beam_spec_MeV-mean_Elosscalo_stop);
...
*/			
			...
			if (kinel) { 
				...
				Fill1DHist(h1d_keff_reco_constEcalo_inel, ke_beam_spec_MeV-mean_Elosscalo_stop);
...
*/
	...
	//save results...
   	TFile *fout = new TFile("mc_ke_advancedKEff.root","RECREATE");
		h1d_kebeam->Write();
		h1d_ke0->Write();
