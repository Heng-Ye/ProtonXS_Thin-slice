...
void BMRW_Uncertainties_Producer() {
        //plot style --------------------------------------------------------------------//
	...
	//List all possible conminations of mu+-dmu and sigma+-dsigma ------------------------//
	...
	//find maximum/minimum curve based on the reference value -----------------------------------//
	...
	TLegend *leg_3 = new TLegend(0.3,0.6,0.7,0.8);
	...
	//Output the bmrw sys function -----------------------------------------------
	//data
	TF1 *kerw_data=new TF1(Form("kerw_data"),fitg, 0, 800, 3);
	kerw_data->SetParameter(0, mu_denom_data);
	kerw_data->SetParameter(1, sg_denom_data);
	kerw_data->SetParameter(2, 1);

	kerw_data->SetParError(0, err_mu_denom_data);
	kerw_data->SetParError(1, err_sg_denom_data);
	kerw_data->SetParError(2, 0);

	//mc
	TF1 *kerw_mc=new TF1(Form("kerw_mc"),fitg, 0, 800, 3);
	kerw_mc->SetParameter(0, mu_nom_mc);
	kerw_mc->SetParameter(1, sg_nom_mc);
	kerw_mc->SetParameter(2, 1);

	kerw_mc->SetParError(0, err_mu_nom_mc);
	kerw_mc->SetParError(1, err_sg_nom_mc);
	kerw_mc->SetParError(2, 0);

	//Save Weighting Func ------------------------------------------------------
	TFile *f_out = TFile::Open("./bmrw_sys.root","recreate");
		kerw_data->Write("kerw_data");
		kerw_mc->Write("kerw_mc");
		kerw[0]->Write("kerw_cen");
		kerw[key_max]->Write("kerw_max");
		kerw[key_min]->Write("kerw_min");
	f_out->Close();
